...
### Plan-3 named requirements (from the final whole-branch review, 2026-08-11)

Binding for plan 3's briefs, not optional carries:
1. **Offline camera capture recoverability** — a CameraPicker photo that fails upload is currently destroyed after one toast (composer clears before attempting; the bytes exist nowhere else). Re-stage dropped attachments into the composer, or persist attachment bytes into the Outbox directory. Must not reach TestFlight unfixed.
2. **Un-share vs sticky-debounce race** — cancel the pending field debounce (Debouncer needs cancel()) and clear the local binding when applying the un-share patch; the UI test currently dodges this with a 2s sleep.
3. **Outbox attempts cap + dead-letter** — 403'd file entries retry forever (per-user scoping makes this visible); cap attempts, surface dead letters.
4. **Extension memory budget** — fileImporter's eager whole-file Data read is a crash under the share extension's ~120MB cap; stream or bound reads before plan 3 reuses the attachment pipeline.
5. **App Group Outbox migration** must preserve per-user directory scoping (Outbox.swift doc comment) and add a cross-process drain claim (per-instance isDraining doesn't span processes).
6. **Server fix (owner-flagged)**: quick-pdf-summary title overwrite (index.ts:96-105 — add the add-note-style guard).
