...
| [composer-remind-me](2026-09-07-composer-remind-me.html) | v0.2 | **2 + 3 chosen**, merged; iOS + share sheet awaiting confirmation | Reminders from the capture surfaces. v0.1 showed three options (bell beside the pin / "Remind me" offer in the input-chip row / phrase recognised in the note). v0.2, after Will chose 2 and 3: the chip row is the reminder's one home on every surface, a recognised phrase fills the same chip, × is the only remove control. Adds the **live recognizer** (type into it): strong / weak intent lexicon × time grammar → set / set-default-tomorrow / offer / nothing, plus the consumed-or-kept rule. Adds the iOS composer and the share sheet (spec's preset row above Save, wired to the note field, done copy names the day). Option 1 kept at the bottom as not chosen. Render: `2026-09-07-composer-remind-me.png`. | `UnifiedInputPanel.tsx`; `CaptureComposerView.swift`; `ShareComposeView.swift`; specs/2026-09-06-reminders-design.md §iOS; ui-changes 2026-09-06 reminders |
