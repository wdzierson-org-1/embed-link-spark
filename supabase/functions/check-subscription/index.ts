...
serve(async (req) => {
  ...
  try {
    ...
    // Auth failures are the caller's problem (expired JWT, dead session), not a
    // server error — return 401 so they don't surface as 500s in logs/alerts.
    const unauthorized = (message: string) => {
      logStep("Auth failed", { message });
    ...
    if (!authHeader) return unauthorized("No authorization header provided");
